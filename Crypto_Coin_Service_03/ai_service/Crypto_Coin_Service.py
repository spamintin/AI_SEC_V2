...
#BTC, ETH, XRP
#COIN FILES prefix_btc_,eth_,xrp_
service_coin = ["BTC","ETH","XRP"]
btc_lstm_model = load_rnnmodel(f"{COIN_PATH}/btc_lstm_model.keras")
eth_lstm_model = load_rnnmodel(f"{COIN_PATH}/eth_lstm_model.keras")
xrp_lstm_model = load_rnnmodel(f"{COIN_PATH}/xrp_lstm_model.keras")
btc_err_rate=""
eth_err_rate=""
xrp_err_rate=""
btc_graph=""
eth_graph=""
xrp_graph=""
def init_server(): #model's error rate preprocessing
    global btc_err_rate
    global eth_err_rate
    global xrp_err_rate
    global btc_graph
...
def input_request(coin_name="BTC",time_gap=1):
    if not coin_name in service_coin:
        return f"{coin_name} has not been on service yet"
    ...
    return {"y_pred":y_predarr,"graph":graph_url,"err_rate":err_rate}
